...
<template>
  <ModalScanQr v-model:show-modal="showQrModal" :self-code="selfCode" />
  <div>
    <p class="leading-loose text-md">Receive files</p>
  </div>
  <div class="my-2">
    <div class="label pb-0">
      <span v-if="checkedURL" class="label-text">Your share URL is:</span>
      <span v-else class="label-text">Your share code is:</span>
      <div class="form-control ">
        <label class="label cursor-pointer">
          <span class="label-text">URL</span>
          <input type="checkbox" class="toggle toggle-primary ml-3" v-model="checkedURL" />
        </label>
      </div>
    </div>
    <label class="input input-bordered flex items-center gap-2">
      <input type="text" class="grow disabled disabled:opacity-60" :placeholder="getShareCodeForReceive()" disabled />
      <button id="scanQr" class="btn btn-ghost btn-circle btn-sm" @click="toggleQrModal">
        <ScanQrIcon />
      </button>
      <button id="copyClipboard" class="btn btn-ghost btn-circle btn-sm" @click="copyToClipboard">
        <CopySuccessIcon v-if="userClicked" />
        <CopyIcon v-else />
      </button>
    </label>
  </div>
  <div v-if="receivedFileDesc" class="mockup-window border bg-base-300">
    <table class="table">
      <thead>
        <tr>
          <th>Name</th>
      ...
      <tbody>
        <tr>
          ...
          <th class="sm:w-36">
            ...
            <button v-if="dataReceivedSize > 0 && dataReceivedSize === receivedFileDesc.size"
              class="btn btn-sm btn-secondary" @click="$emit('saveFile')">
              Download
            </button>
            <progress v-else class="progress sm:w-36" :value="dataReceivedSize" :max="receivedFileDesc.size" />
          </div>
          </th>
        </tr>
      </tbody>
